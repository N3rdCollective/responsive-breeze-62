  ...
  const handleQuotePost = async (postToQuote: ForumPost) => {
    ...
    if (postToQuote.user_id !== user.id) {
      const notificationData = {
        ...
        actor_id: user.id,
        type: 'quote' as const, // Changed from 'generic' to 'quote'
        topic_id: topic.id,
        post_id: postToQuote.id, // This is the ID of the post being quoted
        // content_summary: `${currentUserDisplayName} quoted your post in "${topic.title}"`, // This will be generated by the mapper
        details: { 
          // true_type field is redundant if `type` is specific like 'quote'
        ...
        // The link_url will be constructed by the notification mapper
