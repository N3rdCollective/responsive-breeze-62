-- Enhanced Staff Data Security Implementation (Fixed)
...
-- Create trigger for staff data modifications (not SELECT)
CREATE OR REPLACE FUNCTION public.audit_staff_modifications()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DROP TRIGGER IF EXISTS staff_modification_audit_trigger ON public.staff;
CREATE TRIGGER staff_modification_audit_trigger
  AFTER UPDATE OR DELETE ON public.staff
  FOR EACH ROW EXECUTE FUNCTION public.audit_staff_modifications();
