-- Enhanced Staff Data Security Implementation (Final Fix)
...
-- Policy 1: Staff can only see their own basic profile (no email access)
...
-- Policy 3: Super admins can manage all staff records
...
-- Policy 4: Regular staff cannot update any staff records (including their own)
-- This prevents email/role self-modification
CREATE POLICY "staff_no_self_updates" ON public.staff
FOR UPDATE
TO authenticated
USING (false);

-- Policy 5: Deny all INSERT/DELETE operations except for super admins
CREATE POLICY "restrict_staff_modifications" ON public.staff
FOR INSERT
TO authenticated
WITH CHECK (
  EXISTS (
...
CREATE POLICY "restrict_staff_deletions" ON public.staff
FOR DELETE
TO authenticated
USING (
  EXISTS (
...
-- Create enhanced security functions with audit logging
...
-- Create function for HR to access sensitive staff data
...
-- Create secure function for staff management list (no emails)
...
-- Create a secure view for staff self-service (very limited fields only)
