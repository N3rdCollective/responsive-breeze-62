...
-- ===========================================
-- 5. AUDIT LOGGING FUNCTION (NO TRIGGER)
-- ===========================================

-- Create audit log function for manual use
CREATE OR REPLACE FUNCTION public.log_profile_access(target_profile_id uuid, action_description text)
RETURNS void AS $$
BEGIN
  -- Only log when staff access other users' profiles
  IF auth.uid() != target_profile_id AND EXISTS(SELECT 1 FROM public.staff WHERE id = auth.uid()) THEN
