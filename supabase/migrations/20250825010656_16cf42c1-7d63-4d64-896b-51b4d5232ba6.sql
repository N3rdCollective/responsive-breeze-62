-- Enhanced Staff Data Security Implementation
-- Phase 1-5: Comprehensive security overhaul for staff table
...
-- Create a secure view for staff members to see limited own data
...
ALTER VIEW public.staff_own_profile SET (security_invoker = true);
...
CREATE TABLE IF NOT EXISTS public.staff_data_audit (
  ...
  access_type text NOT NULL, -- 'read', 'update', 'delete', 'sensitive_read'
...
-- Policy 1: Staff can only see their own basic profile
...
-- Policy 2: HR personnel can read staff data (excluding super sensitive fields)
...
WITH CHECK (
  id = auth.uid() 
  AND OLD.email = NEW.email  -- Email cannot be changed
  AND OLD.role = NEW.role    -- Role cannot be self-changed
  AND OLD.hr_permissions = NEW.hr_permissions -- HR permissions cannot be self-changed
);
...
FOR INSERT
TO authenticated
WITH CHECK (true); -- System can always insert audit records

-- Create trigger for comprehensive staff data access logging
...
BEGIN
  ...
  IF TG_OP = 'SELECT' THEN
    INSERT INTO public.staff_data_audit (
      ...
      NEW.id,
      'read',
      ARRAY['id', 'email', 'role', 'hr_permissions', 'created_at', 'updated_at'],
      inet_client_addr()
    );
...
DROP TRIGGER IF EXISTS staff_access_audit_trigger ON public.staff;
CREATE TRIGGER staff_access_audit_trigger
  AFTER SELECT OR UPDATE OR DELETE ON public.staff
