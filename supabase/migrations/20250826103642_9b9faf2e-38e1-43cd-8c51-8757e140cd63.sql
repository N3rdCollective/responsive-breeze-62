-- Enhanced Profile Security Implementation
-- 1. Create role-specific data views for staff moderation

-- Create a secure view for staff moderation (no sensitive data)
CREATE OR REPLACE VIEW public.staff_moderation_profiles AS 
SELECT 
  id,
  username,
  display_name,
...
-- Grant access to authenticated users (will be further restricted by RLS)
GRANT SELECT ON public.staff_moderation_profiles TO authenticated;

-- 2. Create access justification table
...
-- 4. Create enhanced logging function
...
-- 5. Create field-level access control function
...
-- Staff moderation view policy
CREATE POLICY "Staff can view moderation profiles"
ON public.staff_moderation_profiles
FOR SELECT TO authenticated
USING (
  EXISTS (SELECT 1 FROM public.staff WHERE id = auth.uid() AND role IN ('moderator', 'admin', 'super_admin'))
);
